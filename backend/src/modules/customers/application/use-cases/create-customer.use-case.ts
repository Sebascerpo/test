...
export type CreateCustomerUseCase = (deliveryInfo: DeliveryInfo) => ResultAsync<Customer>;
...
export const createCustomerUseCase = (
  ...
  return async (deliveryInfo: DeliveryInfo) => {
    try {
      ...
      if (!deliveryInfo.fullName || !deliveryInfo.email || !deliveryInfo.address) {
        return err(new Error('Missing required delivery information'));
      }

      // Check if customer exists
      const existingCustomer = await customerRepository.findByEmail(deliveryInfo.email);
      if (existingCustomer) {
