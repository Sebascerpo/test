...
export function ProductCatalog({ onSelectProduct }: ProductCatalogProps) {
  ...
  const { selectedProduct, transactionResult } = useAppSelector((state) => state.payment);
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Get updated stock from transaction result
  const updatedStock = transactionResult?.status === 'APPROVED' && selectedProduct 
  ...
  const getStockStatus = (stock: number) => {
    if (stock <= 0) return { label: 'Agotado', color: 'destructive', progress: 0 };
    if (stock <= 5) return { label: `¡Solo ${stock}!`, color: 'warning', progress: Math.min((stock / 30) * 100, 100) };
    if (stock <= 10) return { label: 'Pocas unidades', color: 'low', progress: Math.min((stock / 30) * 100, 100) };
        ...
        <div className="bg-gradient-to-r from-primary/5 via-primary/10 to-transparent rounded-lg p-3 border border-primary/10">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm">
              ...
              <span className="text-muted-foreground"> en compras mayores a $300.000</span>
            </p>
          </div>
        </div>
      </motion.div>
                ...
                <div className="relative w-full aspect-[4/3] bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  {product.imageUrl ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <PackageIcon size={48} className="text-slate-300 dark:text-slate-600" />
                    </div>
                  )}
